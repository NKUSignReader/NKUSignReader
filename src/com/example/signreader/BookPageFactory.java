...
public class BookPageFactory {
	...
	public int returnBufBegin()
	{
		return m_mbBufBegin;
	}
	
	public int returnBufEnd()
	{
		return m_mbBufEnd;
	}
	...
	public BookPageFactory(int w, int h, int fontsize, int c, Context context1) {
		// TODO Auto-generated constructor stub
		mWidth = w;
		mHeight = h;
		mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		...
		context = context1;// 可显示的行数
	}

	public void openbook(String strFilePath, int Buflen) throws IOException {
		...
		m_mbBufEnd = Buflen;
		Log.e("openbook",String.valueOf(m_mbBufLen));
	}
	
	public void openbookFirstTime(String strFilePath) throws IOException {
		book_file = new File(strFilePath);
		long lLen = book_file.length();
		m_mbBufLen = (int) lLen;
		m_mbBuf = new RandomAccessFile(book_file, "r").getChannel().map(
		...
		bookoperations.open();
		Log.e("length",String.valueOf(m_mbBufLen));
		Book book = new Book(strFilePath,m_mbBufLen,"godfather", 10, 0);
		bookoperations.insert(book);
	...
	public void Fix_onDraw(Canvas c, int height, int width) {
		if (m_lines.size() == 0)
			m_lines = pageDown();
		if (m_lines.size() > 0) {
			if (m_book_bg == null)
				...
				RectF = new Rect(0, 0, width, height); 
				c.drawBitmap(m_book_bg, null, RectF, null);
			int y = marginHeight;
